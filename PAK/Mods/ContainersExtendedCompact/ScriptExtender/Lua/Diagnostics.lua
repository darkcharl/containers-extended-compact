...
--
-- Listener 0 — AddedTo (player inventory, after Osiris rules ran):
--   Fires when a non-story, non-container item lands in a player inventory.
--   Two cases are logged:
--   a) Item has NO known sort tag → <<CATCH-ALL>> / <<NEEDS PATCH?>>
--      These fall through to the BoH catch-all rule.  Raw tag UUIDs and
--      proficiency group checks are dumped to identify missing SORT_TAGS entries.
--   b) Item HAS a known sort tag but is STILL in the player inventory after all
--      Osiris rules ran → <<SORT FAILED>>.  Indicates the target container was
--      absent OR rejected the item (e.g. CAMPSUPPLIES + rotten eggs).  The Lua
--      CAMPSUPPLIES_Fallback in Stacking.lua handles case (b) for that tag.
--   The GetDirectInventoryOwner check filters out false positives: items that
--   were successfully moved to a sub-container during the Osiris rule pass.
--
-- Listener 1 — AddedTo:
...
Ext.Osiris.RegisterListener("AddedTo", 3, "after", function(item, inventory, _)
    ...
    if not isCatchAll then
        ...
        -- AND the item is still there after Osiris rules ran.
        -- GetDirectInventoryOwner returns a bare UUID; bareUUID() normalises inventory
        -- for comparison.  If they differ the sort rule moved the item successfully.
        ...
        if Osi.DB_Players:Get(nil) then
            for _, row in pairs(Osi.DB_Players:Get(nil)) do
                if row[1] == inventory then
                    ...
                    log(string.format("Pickup_KnownTag_Unsorted | entity=%-60s | tpl=%s | tags=%s <<SORT FAILED>>",
                        item, tpl, tags))
